...
if (isset($tpl['status']))
{
	...
	pjUtil::printNotice(__('infoSubmissionsTitle', true), __('infoSubmissionsDesc', true));
			...
			if ($count > 0)
			{
				...
				for ($i = 0; $i < $count; $i++)
				{
					...
					<tr class="pj-table-row-odd">
						<td>
							<?php
							if($tpl['arr'][$i]['key'] == 'o_owner_is_active'){
								
								?>
								<span class="title-tooltip"><?php __('opt_' . $tpl['arr'][$i]['key']);?></span>&nbsp;<a href="#" class="pj-form-langbar-tip center-langbar-tip" title="<?php __('lblOwnerActiveTip'); ?>"></a>
								<?php
							} else{
						...
						<td class="tblError" colspan="2">
							...
							switch ($tpl['arr'][$i]['type'])
							{
									...
									switch ($tpl['arr'][$i]['key'])
									{
										case 'o_paypal_address':
											...
											<span class="pj-form-field-custom pj-form-field-custom-before">
												...
												<input type="text" name="value-<?php echo $tpl['arr'][$i]['type']; ?>-<?php echo $tpl['arr'][$i]['key']; ?>" class="pj-form-field w200" value="<?php echo pjSanitize::html($tpl['arr'][$i]['value']); ?>" />
											</span>
											<?php
											break;
										default:
								...
								case 'int':
									...
									break;
								case 'float':
								    ?><input type="text" name="value-<?php echo $tpl['arr'][$i]['type']; ?>-<?php echo $tpl['arr'][$i]['key']; ?>" class="pj-form-field field-float w60 number" value="<?php echo pjSanitize::html($tpl['arr'][$i]['value']); ?>" /><?php
									break;
								case 'enum':
									...
									{
										$option_text = $el;
										if($tpl['arr'][$i]['key'] == 'o_allow_add_property' || $tpl['arr'][$i]['key'] == 'o_owner_is_active')
										{
											$option_text = $el == 'Yes' ? $_yesno['T']: $_yesno['F'];
										}
										if ($default[1] == $el)
										{
										    ?><option value="<?php echo $default[0].'::'.$el; ?>" selected="selected"><?php echo pjSanitize::html($option_text); ?></option><?php
										} else {
										    ?><option value="<?php echo $default[0].'::'.$el; ?>"><?php echo pjSanitize::html($option_text); ?></option><?php
										}
									}
									?>
									</select>
				...
				<tr class="pj-table-row-odd">
					<td colspan="3">
					<?php
					pjUtil::printNotice(null,__('opt_period_note', true), true); 
					?></td>
				</tr>
				<tr class="pj-table-row-even">
					<td class="bold"><?php __('payment_period'); ?></td>
					<td class="bold" colspan="2"><?php __('payment_price'); ?></td>
				</tr>
				<?php
