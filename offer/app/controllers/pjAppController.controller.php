...
class pjAppController extends pjController
{
	...
	public $defaultCheckLocale = 'admin_check_locale_id';
    ...
    public function beforeFilter()
    {
    	...
    	if ($_GET['controller'] != 'pjInstaller')
		{
			...
			$force = false;
			if ($_GET['controller'] != 'pjLocale')
			{
				if(!isset($_SESSION[$this->defaultCheckLocale]))
				{
					$_SESSION[$this->defaultCheckLocale] = $_SESSION[$this->defaultLocale];
				}else{
					if($_SESSION[$this->defaultCheckLocale] != $_SESSION[$this->defaultLocale])
					{
						$force = true;
						$_SESSION[$this->defaultCheckLocale] = $_SESSION[$this->defaultLocale];
					}
	...
	public function getAdminEmail()
	{
		$arr = pjUserModel::factory()
			...
			->limit(1)
			->getData();
		return !empty($arr) ? (!empty($arr[0]['email']) ? $arr[0]['email'] : null) : null;
	}
