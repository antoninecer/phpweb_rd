...
{	
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if (isset($_POST['feature_create']))
			{
				...
				if ($id !== false && (int) $id > 0)
				{
					...
					if (isset($_POST['i18n']))
					{
						pjMultiLangModel::factory()->saveMultiLang($_POST['i18n'], $id, 'pjFeature');
					}
				} else {
					$err = 'AF04';
				}
	...
	public function pjActionDeleteFeature()
	{
		...
		if ($this->isXHR())
		{
			...
			if (pjFeatureModel::factory()->setAttributes(array('id' => $_GET['id']))->erase()->getAffectedRows() == 1)
			{
				...
				pjPropertyFeatureModel::factory()->where('foreign_id', $_GET['id'])->eraseAll();
				$response['code'] = 200;
			} else {
				$response['code'] = 100;
			}
	...
	public function pjActionDeleteFeatureBulk()
	{
		...
		if ($this->isXHR())
		{
			if (isset($_POST['record']) && count($_POST['record']) > 0)
			{
				...
				pjPropertyFeatureModel::factory()->whereIn('feature_id', $_POST['record'])->eraseAll();
			}
		}
		exit;
	}
	...
	public function pjActionGetFeature()
	{
		...
		if ($this->isXHR())
		{
			$pjFeatureModel = pjFeatureModel::factory()
				->join('pjMultiLang', "t2.foreign_id = t1.id AND t2.model = 'pjFeature' AND t2.locale = '".$this->getLocaleId()."' AND t2.field = 'name'", 'left');
			
			if (isset($_GET['q']) && !empty($_GET['q']))
			{
				$q = pjObject::escapeString($_GET['q']);
				$pjFeatureModel->where('t2.content LIKE', "%$q%");
			}
			...
			$data = $pjFeatureModel->select('t1.id, t1.category_id, t1.status, t2.content AS name')
				->orderBy("$column $direction")->limit($rowCount, $offset)->findAll()->getData();
			
			$feature_categories = __('feature_categories', true, false);
			foreach($data as $k => $v)
			{
				$v['category'] = $feature_categories[$v['category_id']];
				$data[$k] = $v;
			}			
	...
	public function pjActionSaveFeature()
	{
		...
		{
			$pjFeatureModel = pjFeatureModel::factory();
			if (!in_array($_POST['column'], $pjFeatureModel->i18n))
			{
				$pjFeatureModel->where('id', $_GET['id'])->limit(1)->modifyAll(array($_POST['column'] => $_POST['value']));
			} else {
				pjMultiLangModel::factory()->updateMultiLang(array($this->getLocaleId() => array($_POST['column'] => $_POST['value'])), $_GET['id'], 'pjFeature');
			}
		}
		exit;
	}
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if (isset($_POST['feature_update']))
			{
				...
				if (isset($_POST['i18n']))
				{
					pjMultiLangModel::factory()->updateMultiLang($_POST['i18n'], $_POST['id'], 'pjFeature');
				}
