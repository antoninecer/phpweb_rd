...
{
	public function pjActionCheckType()
	{
		$this->setAjax(true);
		...
		if ($this->isXHR() && isset($_POST['locale']))
		{
			...
			{
				$pjTypeModel->where('t1.id !=', $_POST['id']);
			}
			$pjTypeModel->where("t1.id IN(SELECT TL.foreign_id FROM `".pjMultiLangModel::factory()->getTable()."` AS TL WHERE TL.model='pjType' AND TL.field='name' AND TL.content = '".$value."' AND TL.locale='$locale')");
			echo $pjTypeModel->findCount()->getData() == 0 ? 'true' : 'false';
		}
		exit;
	}
	...
	public function pjActionStatusType()
	{
		...
		if ($this->isXHR())
		{
			...
			{
				pjTypeModel::factory()->whereIn('id', $_POST['record'])->modifyAll(array('status' => $_GET['status']));
			}
		}
		exit;
	}
	...
	public function pjActionSaveType()
	{
		...
		{
			$pjTypeModel = pjTypeModel::factory();
			if (!in_array($_POST['column'], $pjTypeModel->i18n))
			{
